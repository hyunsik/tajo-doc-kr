...
Character-separated values 파일은 로우와 컬럼으로 구성된 표로 나타내어진 데이터 셋입니다.
...
=======================================================================
Tajo에서 어떻게 TEXT 포맷을 사용한 테이블을 만드나요?
=======================================================================
...
아래는 TEXT 파일을 사용해 테이블을 만드는 방법에 대한 예제입니다.
...
..주의하세요::
  ...
  ``\n`` 을 구분자로 사용할 때 주의하셔야 합니다. 이는 TEXT 파일 포맷이 ``\n`` 를 레코드의 라인 구분자로 사용하기 때문입니다.
...
TEXT 저장 포맷은 여러분이 작성한 (역)직렬화 클래스를 적용할 수 있어 TEXT 데이터를 읽고 쓰기 위한 인터페이스를 제공하는것 뿐만 아니라, 일반 텍스트 파일 포맷에 대한 처리 방법도 제공할 수 있습니다.
예를 들어, (역)직렬화 클래스를 새로 작성해 Tajo를 통해 JSON 파일 포맷 뿐만 아니라 특별한 텍스트 파일 또한 처리 할 수 있습니다.

여러분이 작성한 (역)직렬화 클래스는  ``text.serde`` 속성에 지정해 주시면 됩니다. 이 속성의 값으로 여러분이 작성하신 (역)직렬화 클래스의 전체 이름을 지정하시면 됩니다.
...
기본적으로, TEXT 파일 내의  ``NULL`` 값은 공백 문자열입니다. (``''``).
...
=========================================
Apache Hive와의 호환성 이슈들
=========================================

Tajo를 통해 만들어진 TEXT 파일은 하이브에서 추가적인 처리 없이 바로 사용 가능합니다.
